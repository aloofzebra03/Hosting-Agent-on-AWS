...
from .core import generate_joke, generate_explanation_with_suggestions, handle_autosuggestion

def create_workflow():
    print("Setting up stateful joke generation workflow with merged explanation and autosuggestions")
    ...
    graph.add_node('generate_explanation_with_suggestions', generate_explanation_with_suggestions)
    graph.add_node('handle_autosuggestion', handle_autosuggestion)
    
    # Add edges - simplified flow since explanation and suggestions are combined
    graph.add_edge(START, 'generate_joke')
    graph.add_edge('generate_joke', 'generate_explanation_with_suggestions')
    
    # After generating explanation+suggestions, route to handler
    # The interrupt after 'generate_explanation_with_suggestions' will pause here
    # User calls /action which resumes and goes to handle_autosuggestion
    graph.add_conditional_edges(
        'generate_explanation_with_suggestions',
    ...
    def route_after_action(state):
        ...
        # If joke was regenerated/enhanced, go back to generate explanation with suggestions
        if status in ['joke_regenerated', 'joke_enhanced', 'similar_joke_generated']:
            return 'generate_explanation_with_suggestions'
    ...
    graph.add_conditional_edges(
        ...
        route_after_action,
        {
            'generate_explanation_with_suggestions': 'generate_explanation_with_suggestions',
            END: END
        }
    )
    ...
    workflow = graph.compile(
        ...
        interrupt_after=['generate_joke', 'generate_explanation_with_suggestions', 'handle_autosuggestion']  # Interrupt after all nodes
    )
    print("Workflow setup completed with merged explanation+suggestions node")
